...
// Find the maximum existing ID and determine the next ID
        ...
        for (String subCategoryId : subCategoryIds) {
            if (subCategoryId != null && subCategoryId.startsWith("CO")) {
                String numberPart = subCategoryId.substring(2);
                try {
...
// Dynamically adjust the number of digits
        ...
        String newSubCategoryId = "CO" + String.format(format, newIdNumber);
